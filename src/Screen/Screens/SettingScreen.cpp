...
SettingScreen::SettingScreen() : Screen(ScreenType::SETTING, "setting")
{
	...
	addElementGroup(make_shared<SliderText>(Vector2(1000, 320), 
		...
		Text("screen.setting.masterVolume", 40, TextAlign::CENTER, 1.0f),
		TextArg(Vector2(1170, 325), "screen.setting.currentMasterVolume", std::map<std::string, argTypes>{ {"volume", &SystemConfig::instance().getMasterVolume()} }, 40, TextAlign::CENTER, 1.0f)
	));
