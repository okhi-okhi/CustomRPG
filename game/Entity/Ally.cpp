...
{
	this->expNext = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.expNext"), { {"level", this->level} }));

	std::map <std::string, int> vars = this->talents;
	vars["level"] = this->level;

	this->hpMax = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.hpMax"), vars))
	+ this->level * this->occupation.getHpMaxScale() + this->race.getHpMaxBase();
	this->hp = this->hpMax;

	this->manaMax = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.manaMax"), vars))
		+ this->level * this->occupation.getManaMaxScale() + this->race.getManaMaxBase();
	this->mana = this->manaMax;

	this->atk = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.atk"), vars));

	this->defense = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.defense"), vars));

	this->luck = static_cast<int>(JsonUtils::readValueFromString(Config::instance().getFormula("entity.luck"), vars))
